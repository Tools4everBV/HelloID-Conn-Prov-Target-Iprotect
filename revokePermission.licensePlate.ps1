#################################################
# HelloID-Conn-Prov-Target-iProtect-Permissions-KeyGroups-LicensePlate-Revoke
# PowerShell V2
#################################################
# Enable TLS1.2
...
try {
    ...
    if (($correlatedLicensePlate | Measure-Object).count -eq 1) {
        $actionLicensePlate = "RevokePermission"
    }
    ...
    switch ($actionLicensePlate) {
        "RevokePermission" {
            #region Get current permission for licenseplate
            try {
                $queryCorrelatePermissionLicensePlate = "
                SELECT
                    KEYKEYGROUPID
                FROM
                    KEYKEYGROUP
                WHERE
                    ...
                    AND ACCESSKEYID = $($correlatedLicensePlate.ACCESSKEYID)
                "

                $correlatePermissionLicensePlateSplatParams = @{
                    BaseUrl    = $actionContext.Configuration.BaseUrl
                    JSessionID = $jSessionID
                    Query      = $queryCorrelatePermissionLicensePlate
                    QueryType  = "query"
                }

                Write-Verbose "Querying permission for licenseplate where [KEYGROUPID] = [$($actionContext.References.Permission.Id)] AND [ACCESSKEYID] = [$($correlatedLicensePlate.ACCESSKEYID)]. SplatParams: $($correlatePermissionLicensePlateSplatParams | ConvertTo-Json)"

                $correlatedPermissionLicensePlate = $null
                $correlatedPermissionLicensePlate = Invoke-IProtectQuery @correlatePermissionLicensePlateSplatParams
            
                Write-Verbose "Queried permission for permission for licenseplate where [$licensePlateCorrelationField] = [$($licensePlateCorrelationValue)]. Result: $($correlatedPermissionLicensePlate | Out-String)"
            }
            catch {
                $ex = $PSItem

                $auditMessage = "Error querying permission for licenseplate where [$licensePlateCorrelationField] = [$($licensePlateCorrelationValue)]. Error: $($ex.Exception.Message)"
                Write-Warning "Error at Line [$($ex.InvocationInfo.ScriptLineNumber)]: $($ex.InvocationInfo.Line). Error: $($ex.Exception.Message)"
                ...
                # Log query
                Write-Warning "Query: $queryCorrelatePermissionLicensePlate"
        
                # Throw terminal error
                throw $auditMessage
            }
            #endregion Get current permission for licenseplate

            if (($correlatedPermissionLicensePlate | Measure-Object).count -eq 1) {
                #region Delete permission for licenseplate
                try {
                    $queryDeletePermissionLicensePlate = "
                    DELETE
                        *
                    FROM
                        KEYKEYGROUP
                    WHERE
                        KEYKEYGROUPID = $($correlatedPermissionLicensePlate.KEYKEYGROUPID)
                    "

                    $deletPermissionLicensePlateSplatParams = @{
                        BaseUrl    = $actionContext.Configuration.BaseUrl
                        JSessionID = $jSessionID
                        Query      = $queryDeletePermissionLicensePlate
                        QueryType  = "update"
                    }

                    if (-Not($actionContext.DryRun -eq $true)) {
                        Write-Verbose "Deleting permission where [KEYKEYGROUPID = $($actionContext.References.Permission.Id)] for licenseplate with AccountReference: $($actionContext.References.Account.LicensePlate | ConvertTo-Json). SplatParams: $($deletPermissionLicensePlateSplatParams | ConvertTo-Json)"

                        $deletedPermissionLicensePlate = $null
                        $deletedPermissionLicensePlate = Invoke-IProtectQuery @deletPermissionLicensePlateSplatParams

                        $outputContext.AuditLogs.Add([PSCustomObject]@{
                                # Action  = "" # Optional
                                Message = "Deleted permission where [KEYKEYGROUPID = $($actionContext.References.Permission.Id)] for licenseplate with AccountReference: $($actionContext.References.Account.LicensePlate | ConvertTo-Json)"
                                IsError = $false
                            })
                    }
                    else {
                        Write-Warning "DryRun: Would delete permission where [KEYKEYGROUPID = $($actionContext.References.Permission.Id)] for licenseplate with AccountReference: $($actionContext.References.Account.LicensePlate | ConvertTo-Json). SplatParams: $($deletPermissionLicensePlateSplatParams | ConvertTo-Json)"
                    }
                }
                catch {
                    $ex = $PSItem
                    ...
                    if ($ex.Exception.Message -like "*Value already exists*" -or $ex.Exception.Message -like "*Kolom waarde is niet uniek*") {
                        ...
                        $auditMessage = "Skipped deleting permission where [KEYKEYGROUPID = $($actionContext.References.Permission.Id)] for licenseplate with AccountReference: $($actionContext.References.Account.LicensePlate | ConvertTo-Json). Reason: Permission for accesskey already exists."
                        ...
                        $auditMessage = "Error deleting permission where [KEYKEYGROUPID = $($actionContext.References.Permission.Id)] for licenseplate with AccountReference: $($actionContext.References.Account.LicensePlate | ConvertTo-Json). SplatParams: $($deletPermissionLicensePlateSplatParams | ConvertTo-Json). Error: $($ex.Exception.Message)"
                        Write-Warning "Error at Line [$($ex.InvocationInfo.ScriptLineNumber)]: $($ex.InvocationInfo.Line). Error: $($ex.Exception.Message)"
                        ...
                        # Log query
                        Write-Warning "Query: $queryDeletePermissionLicensePlate"

                        # Throw terminal error
                        throw $auditMessage
                    }
                }
                #endregion Delete permission for licenseplate
            }
            else {
                $auditMessage = "Skipped deleting permission where [KEYKEYGROUPID = $($actionContext.References.Permission.Id)] for licenseplate with AccountReference: $($actionContext.References.Account.LicensePlate | ConvertTo-Json). Reason: Permission for accesskey already no longer."
        ...
        "NotFound" {
            $auditMessage = "Skipped deleting permission where [KEYKEYGROUPID = $($actionContext.References.Permission.Id)] for licenseplate with AccountReference: $($actionContext.References.Account.LicensePlate | ConvertTo-Json). Reason: No licenseplate found where [$($correlationField)] = [$($correlationValue)]. Possibly indicating that it could be deleted, or not correlated."
