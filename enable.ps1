...
try {
    ...
    if ($null -ne $correlatedAccount) {
        Write-Information "Getting linked  accessKey Cards of employee [$($actionContext.References.Account)]"
    ...
    switch ($action) {
        'EnableAccount' {
            foreach ($accessKey in $accessKeyList) {
                ...
                if (-not($actionContext.DryRun -eq $true)) {
                    Write-Information "Enabling IProtect AccessKey with AccessKeyId: [$($accessKey.AccessKeyId)] RCN: [$($accessKey.RCN)]"
                    if (-not ($accessKey.valid -eq 1)) {
                        $splatUpdateAccessKey = @{
                            JSessionID = $jSessionID
                            WebSession = $webSession
                            Query      = $queryUpdateAccessKey
                            QueryType  = 'update'
                        }

                        $null = Invoke-IProtectQuery @splatUpdateAccessKey
                    }
                }
                else {
                    Write-Information "[DryRun] Enable IProtect accessKey with AccessKeyId: [$($accessKey.AccessKeyId)] RCN: [$($accessKey.RCN)], will be executed during enforcement"
                }
                $outputContext.AuditLogs.Add([PSCustomObject]@{
                        Message = "AccessKey with AccessKeyId: [$($accessKey.AccessKeyId)] RCN: [$($accessKey.RCN)] was successfully enabled"
                        IsError = $false
                    })
            }
        ...
        'NoAccessKeysFound' {
            Write-Information "IProtect account: [$($actionContext.References.Account)] has no linked AccessKeys. Skipping enabling AccessKey(s)"
            $outputContext.Success = $true
            $outputContext.AuditLogs.Add([PSCustomObject]@{
                    Message = "IProtect account: [$($actionContext.References.Account)] has no linked AccessKeys. Skipping enabling AccessKey(s)"
                    IsError = $false
                })
            break
        }
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditMessage = "Could not enable IProtect account. Error: $($errorObj.FriendlyMessage)"
        Write-Warning "Error at Line '$($errorObj.ScriptLineNumber)': $($errorObj.Line). Error: $($errorObj.ErrorDetails)"
    }
    else {
        $auditMessage = "Could not enable IProtect account. Error: $($_.Exception.Message)"
