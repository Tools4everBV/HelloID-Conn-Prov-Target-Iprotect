#################################################
# HelloID-Conn-Prov-Target-iProtect-Permissions-KeyGroups-LicensePlate
# PowerShell V2
#################################################
# Enable TLS1.2
...
try {
    ...
    #region Get keygroups for licenseplate
    try {
        $queryGetKeyGroupsLicensePlate = "
        SELECT
            KEYGROUPID,
            NAME
        FROM
        ...
        $getKeyGroupsLicensePlateSplatParams = @{
            BaseUrl    = $actionContext.Configuration.BaseUrl
            JSessionID = $jSessionID
            Query      = $queryGetKeyGroupsLicensePlate
            QueryType  = "query"
        }

        Write-Verbose "Querying keygroups for licenseplate. SplatParams: $($getKeyGroupsLicensePlateSplatParams | ConvertTo-Json)"

        $keyGroupsLicensePlate = $null
        $keyGroupsLicensePlate = Invoke-IProtectQuery @getKeyGroupsLicensePlateSplatParams

        #region GK - Filter for reference type 15 as this only used for access key of type license plate with parking gate authorization
        $keyGroupsLicensePlate = $keyGroupsLicensePlate | Where-Object { $_.KEYGROUPID -eq 15 }
        #endregion GK - Filter for reference type 15 as this only used for access key of type license plate with parking gate authorization
        
        Write-Information "Queried keygroups for licenseplate. Result count: $(($keyGroupsLicensePlate | Measure-Object).Count)"
    }
    catch {
        $ex = $PSItem

        $auditMessage = "Error querying keygroups for licenseplate. Error: $($ex.Exception.Message)"
        Write-Warning "Error at Line [$($ex.InvocationInfo.ScriptLineNumber)]: $($ex.InvocationInfo.Line). Error: $($ex.Exception.Message)"
        ...
        # Log query
        Write-Warning "Query: $queryGetKeyGroupsLicensePlate"
        
        # Throw terminal error
        throw $auditMessage
    }
    #endregion Get keygroups for licenseplate 
}
catch {
    $ex = $PSItem
    ...
    # Send results
    foreach ($keyGroupLicensePlate in $keyGroupsLicensePlate) {
        # Shorten DisplayName to max. 100 chars
        $displayName = "KeyGroup - LicensePlate - $($keyGroupLicensePlate.NAME)"
        $displayName = $displayName.substring(0, [System.Math]::Min(100, $displayName.Length)) 
        $permission = @{
            DisplayName    = $displayName
            Identification = @{
                Id   = $keyGroupLicensePlate.KEYGROUPID
                Name = $keyGroupLicensePlate.NAME
                Type = "KeyGroup"
            }
        }
